...
def GLF(logits, labels, alpha_p, alpha_n, beta_sq):
    ...
    y_tp = tf.keras.backend.sum(y_true * y_pred)
    y_fp = tf.keras.backend.sum(y_pred * (1 - y_true))
    y_fn = tf.keras.backend.sum((1 - y_pred) * y_true)
    ...
    up_ap = 1.0*c2 + 0.9975*c7 + 0.995*c8 + 1.0025*c11 + 1.005*c12 
    ...
    up_an = 1.0*c2 + 0.9975*c7 + 0.995*c8 + 1.0025*c11 + 1.005*c12 + 1.0*c15
    ...
    up_bsq = 1.0*c2 + 0.9975*c7 + 0.995*c8 + 1.0025*c11 + 1.005*c12
